...
public protocol SwipeableTableViewControllerDelegate {
    func didLoadViewControllerAtIndex(_ index: Int) -> UIViewController
}

open class SwipeableTableViewController: UIViewController {
    ...
    open var delegate: SwipeableTableViewControllerDelegate?
    ...
    fileprivate func setupSegmentBarButtons() {
        if let buttonList = titleBarDataSource {
            ...
            for i in 0 ..< buttonList.count {
                ...
                segmentButton.tag = i
                segmentButton.addTarget(self, action: #selector(SwipeableTableViewController.didSegmentButtonTap(_:)), for: .touchUpInside)
